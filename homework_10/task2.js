...
function fighter(character) {
  ...
  method.getName = function() {return stats.name;};
  method.getCombatHistory = function() {return combatHistory;};
  method.getStats = function() {return stats;};
  method.fight = function(contest) {
    if(contest.block()){
      contest.getStats().hp -= stats.attack;
      if(contest.getStats().hp <= 0){
        contest.getStats().hp = 0;
        contest.getCombatHistory().loses++;
        combatHistory.wins++;
      }
      return true;
    } else {
